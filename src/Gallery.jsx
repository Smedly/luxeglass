  ...
  return (
    <article className="bg-white rounded overflow-hidden shadow-sm">
      ...
      <div className="p-3 text-sm">
        ...
        <div className="text-xs text-gray-500">{img.project || 'Studio Boogers'} • {img.location || ''}</div>
      </div>
    </article>
  )
}
...
export default function Gallery({ initialCategory = 'All', initialCount = 9 }) {
  // allow passing other categories later (Showers, Sinks)
  const items = tableGallery // current data source
    ...
    <section className="max-w-6xl mx-auto px-6 py-8">
      ...
      {visible.length > count && (
        <div className="mt-6 text-center">
          <button onClick={() => setCount(c => c + initialCount)} className="px-4 py-2 border rounded">Load more</button>
        </div>
      )}
          ...
          <div className="relative max-w-4xl w-full bg-white rounded shadow-lg overflow-hidden">
            <div className="md:flex">
              <div className="md:w-2/3 p-4">
                <img src={open.large} alt={open.alt} className="w-full h-auto object-contain" />
              </div>
              <div className="md:w-1/3 p-6">
                <h3 className="font-bold text-xl">{open.title}</h3>
                <div className="text-sm text-gray-600 mt-2">{open.project || 'Studio Sample'} {open.location ? `• ${open.location}` : ''}</div>
                <p className="mt-4 text-sm">{open.description}</p>
                <div className="mt-6 flex gap-3">
                  <a href={`mailto:hello@luxeglass.pro?subject=Inquiry about ${encodeURIComponent(open.title)}`} className="px-4 py-2 bg-indigo-600 text-white rounded">Request quote</a>
                </div>
              </div>
            </div>

            <button onClick={() => setOpen(null)} className="absolute top-3 right-3 text-gray-600">Close</button>
          </div>
        </div>
      )}
    </section>
